...
Side::Side(Vector center, float side_length, Color color, char orientation, int i, int j)
	...
	// calculates the 4 vertices, based of the given side and the given subdivision i,j
	switch (orientation) {
		case 'f': case 'b': {
		...
		case 't': case 'd': {
			...
			ij00 = Vector(i, 0, j); ij01 = Vector(i, 0, j + 1);
			ij11 = Vector(i + 1, 0, j + 1); ij10 = Vector(i + 1, 0, j);
			break;
		}
		case 'l': case 'r': {
			...
			offset2 = Vector(0, 0, subside_len);
			ij00 = Vector(0, i, j); ij01 = Vector(0, i + 1, j);
			ij11 = Vector(0, i + 1, j + 1); ij10 = Vector(0, i, j + 1);
			break;
		}
		default:
...
std::array<Vector, 4> Side::get_vertices() const {
